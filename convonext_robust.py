...
from patch_loader_random_robust import patchdataset
from patch_loader_random_robust import patchdataset_test
...
out='./model_wt/convonext_robust/oct8_sm0.2'
if not os.path.exists(out):
    os.makedirs(out)

PATH = os.path.join(out,'convnext.pth')
...
class LabelSmoothingCrossEntropy(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        targets = (1 - self.smoothing) * targets + self.smoothing / pred.size(1)
...
train_dataset = patchdataset(path='/workspace/hnsc_for_tumor/tum_ntum/tum_ntum_new/hpv_data_train.csv',transforms=torch_transform)
val_dataset = patchdataset_test(path='/workspace/hnsc_for_tumor/tum_ntum/tum_ntum_new/hpv_data_test.csv' ,transforms=torch_transform1)
...
model.to(device)
# Use this loss function in place of CrossEntropyLoss
criterion = LabelSmoothingCrossEntropy(smoothing=0.2)  # Here smoothing can be tuned
